...
export class WorkerPlugin {
  static preProcessing(): Plugin {
    return {
      name: "naxt:worker-pre-processing-plugin",
  ...
  static handleJS(): Plugin {
    ...
    return {
      name: "naxt:worker-js-processing-plugin",
  ...
  static postProcessing(): Plugin {
    const licenseRegex = /\/\*\*\n\s*\*\s*@license[\s\S]*?\*\//;

    return {
      name: "naxt:worker-post-processing-plugin",
